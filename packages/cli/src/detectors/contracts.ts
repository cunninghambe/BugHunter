...
export const DETECTOR_CONTRACTS: ReadonlyArray<DetectorContract> = [
  ...
  ...(['network_fault_unhandled', 'network_fault_optimistic_no_revert'] as const).map(kind => ({
    kind,
    requires: {
      phases: ['validate', 'execute', 'classify', 'cluster'] as RequiredPhase[],
      tools: ['browser-mcp'] as ('browser-mcp')[],
    ...
    fixture: { path: 'network-fault-mini', servesKinds: [kind] },
    defaultBudgetMs: 30_000,
    note: `V56.4 browser harness. Fixture pushes (preState, postState, fault, ...) via bootstrap helper; harness dispatches through production ${kind === 'network_fault_unhandled' ? 'detectNetworkFaultUnhandled' : 'detectOptimisticNoRevert'}. Production drives this from a network-fault-injection runner.`,
  })),
  ...(['service_worker_stale', 'web_worker_error', 'webrtc_ice_failure'] as const).map(kind => ({
    kind,
    requires: {
      phases: ['validate', 'execute', 'classify', 'cluster'] as RequiredPhase[],
      tools: ['browser-mcp'] as ('browser-mcp')[],
    ...
    fixture: { path: 'browser-platform-probe-mini', servesKinds: [kind] },
    defaultBudgetMs: 30_000,
    note: `V56.4 browser harness. Fixture pushes synthesized detection objects via window.__bh.pushBrowserPlatformDetection; harness mirrors the production inline rule from browser-platform-probe.ts. Production drives via runtime page-side probes.`,
  })),
  {
    kind: 'visual_anomaly',
    requires: {
    ...
    fixture: { path: 'visual-anomaly-mini', servesKinds: ['visual_anomaly'] },
    defaultBudgetMs: 30_000,
    note: 'V56.4 browser harness. Fixture pushes anomaly objects via window.__bh.pushVisualAnomaly; harness emits one detection per valid-severity anomaly. Production reads anomaly outputs from a vision API.',
  },
  {
    ...
    fixture: { path: 'prompt-injection-mini', servesKinds: ['prompt_injection_executed'] },
    defaultBudgetMs: 30_000,
    note: 'V56.4 browser harness. Fixture pushes (probe, response) via window.__bh.pushPromptInjectionProbe; harness dispatches through production detectPromptInjection.',
  },
  {
    ...
    fixture: { path: 'rtl-mini', servesKinds: ['i18n_rtl_layout_break'] },
    defaultBudgetMs: 30_000,
